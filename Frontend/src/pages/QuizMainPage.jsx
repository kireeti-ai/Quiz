...
import { fetchAllQuizzes, createNewQuiz, deleteQuizById } from '../services/api';

export default function QuizMainPage() {
  const [quizzes, setQuizzes] = useState([]);
  const [showCreateModal, setShowCreateModal] = useState(false);
  const [newQuiz, setNewQuiz] = useState({ title: '', category: '', numQ: 10 }); // Matched 'numQ' to backend
  const navigate = useNavigate();

  // Function to load quizzes from the backend
  const loadQuizzes = () => {
    fetchAllQuizzes()
      .then(response => {
        // The backend Quiz object has 'questions' as an array.
        // We'll calculate the length for the UI.
        const quizzesWithCount = response.data.map(q => ({
          ...q,
          questionsCount: q.questions ? q.questions.length : 0,
          // Use a simple date for display
    ...
    if (newQuiz.title && newQuiz.category && newQuiz.numQ > 0) {
      createNewQuiz(newQuiz.title, newQuiz.category, newQuiz.numQ)
        .then(() => {
          ...
          loadQuizzes(); // Refresh the list from the backend
        })
        .catch(error => {
  ...
  // Calculate stats from the dynamic data
              ...
              return (
                ...
                >
                  <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                    <button className="p-1.5 bg-white border-2 border-stone-800 hover:bg-yellow-200" title="Edit (Not Implemented)">
